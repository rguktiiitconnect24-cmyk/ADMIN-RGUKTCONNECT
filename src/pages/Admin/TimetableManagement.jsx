                                        ...
                                        for (let i = 0; i < 7; i++) {
                                            ...
                                            cells.push(
                                                <td key={`${day}-${i}`} colSpan={colSpan} className="p-0 border border-[var(--color-border)] relative group">
                                                    ...
                                                    >
                                                        {showMergeBtn && (
                                                            <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 flex items-center gap-2 z-20 bg-white dark:bg-slate-800 p-1.5 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 animate-slide-up">
                                                                <button className="bg-indigo-600 text-white hover:bg-indigo-700 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); handleMerge(); }}><Link size={14} /> Merge</button>
                                                                <button className="bg-red-500 text-white hover:bg-red-600 px-2.5 py-1.5 rounded-lg flex items-center justify-center transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); setSelectedCells([]); }}><X size={14} strokeWidth={3} /></button>
                                                            </div>
                                                        )}
                                                        {showUnmergeBtn && (
                                                            <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 flex items-center gap-2 z-20 bg-white dark:bg-slate-800 p-1.5 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 animate-slide-up">
                                                                <button className="bg-indigo-600 text-white hover:bg-indigo-700 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); handleUnmerge(day, i, colSpan); }}><Unlink size={14} /> Unmerge</button>
                                                                <button className="bg-red-500 text-white hover:bg-red-600 px-2.5 py-1.5 rounded-lg flex items-center justify-center transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); setSelectedCells([]); }}><X size={14} strokeWidth={3} /></button>
                                                            </div>
                                                        )}
